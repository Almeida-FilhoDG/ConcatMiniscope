...
% Shan wrote:
% status=rmdir('SHtemp\msvideo_source_extraction', 's')
%% 
...
%processExperimentVideos_DA(expInfo);
...
datetime
%% Shan: view spatial footprints of the cells found while performing CNMFE extraction
%imagesc(max(ms.SFPs,[],3)); %plot all cells heatmap
%ms.rejectedUnits=selectCells(ms);  % manually select cells by shape and traces
